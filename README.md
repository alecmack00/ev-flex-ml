...
- $P_{\text{peak}}$: Peak aggregate feeder demand across the planning horizon (€/kW demand charge penalty $\alpha = 2.5$).

#### Operational Constraints

1. **Feeder Transformer Capacity Limit**:
   $$\sum_{i=1}^{N} P_{i,t} \le P_{\text{feeder, max}} \cdot \eta_{\text{safety}}, \quad \forall t \in \{1, \dots, T\}$$
